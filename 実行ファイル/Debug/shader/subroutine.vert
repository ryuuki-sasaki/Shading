...
subroutine uniform ShadeModelType shadeModel;	//関数ポインタとして使用するユニフォーム変数
...
{
	vec3 s = normalize(vec3(light.position - position));	//光源に対する正規化方向
	return light.ld * material.kd *  max(dot(s, norm), 0.0);	//ディフューズ成分計算
}
...
void main()
{
	...
	//ライティング方程式を評価
	lightIntensity = shadeModel(eyePosition, eyeNorm);	//shadeModelに応じてサブルーチン関数のいずれかにバインド
